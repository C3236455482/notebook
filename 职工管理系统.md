...
![1546398236081](D:\typora\Note\职工管理系统.assets\1546398236081.png)
...
如果想在程序中维护这个不定长度的数组，可以将数组创建到堆区，并利用Worker **的指针(指向指针的指针)维护

**(利用多态统一指针)**

![1546399491099](D:\typora\Note\职工管理系统.assets\1546399491099.png)
...
当用ifs >> c读取一个字符时，ifs会从文件中读取一个字节，并将其转换为char型，然后赋值给变量c。
同时，ifs会将文件指针向前移动一个位置，指向下一个字节。如果读取成功，ifs >> c会返回一个引用到ifs的表达式，该表达式可以隐式转换为bool型，其值为true。如果读取失败，例如到达文件末尾或者遇到错误，ifs >> c会返回一个引用到ifs的表达式，该表达式可以隐式转换为bool型，其值为false。因此，您可以用while (ifs >> c)作为循环条件，来判断是否继续读取字符
...
WorkerManager::WorkerManager()
{
	...
	//文件不存在情况
	if (!ifs.is_open())
 //ifs.is_open是一个函数，用于检查一个流是否已经与一个文件关联。这个函数不需要读取文件的内容，只需要检查流的内部状态，看是否有一个有效的文件句柄
	{
...
```C++
	...
	char ch;
	ifs >> ch;
/*这两行代码是必须要有的，因为它们是判断文件是否为空的关键步骤。如果没有这两行代码，那么ifs.eof() 就无法正确地返回true或false，而是一直返回false，因为没有尝试过从文件中读取任何字符。这样就无法判断文件是否为空了。
每次读字符ifs读到的字符会被记录在一个内部的缓冲区中，这个缓冲区是一个字节数组，用来存储从文件中读取的数据。ifs.eof()是通过检查这个缓冲区中的位置来判断是否到达文件的结束位置的。*/
	if (ifs.eof())
	{
		cout << "文件为空!" << endl;
		this->m_EmpNum = 0;
...
int WorkerManager::get_EmpNum()
{
	...
	int num = 0;
	/*ifs对于这些空格的处理是，它会把空格作为分隔符，用来区分不同的数据。
	当你使用ifs >> id时，它会跳过所有的空格，直到遇到一个非空格的字符，然后尝试把它转换成整数。
	如果成功，就把这个整数赋给id，并且把输入流的位置移动到下一个空格或者文件末尾。
	如果失败，就设置输入流的错误状态，并且不改变输入流的位置和id的值。*/
...
void WorkerManager::Del_Emp()
{
	...
	else
	{
		...
		if (index != -1)  //说明index上位置数据需要删除
		{
            ...
            /*使用了ios::out作为ofstream的打开模式，这意味着每次打开文件时，都会清空文件的内容，然后写入新的数据。
            如果想要在文件末尾追加数据，而不是覆盖原有的数据，你需要使用ios::app模式，或者在ios::out后面加上ios::ate标志。
            这两种模式都会将文件指针定位到文件末尾，然后写入数据。*/
			cout << "删除成功！" << endl;
		}
		else
		{
...
void WorkerManager::Mod_Emp()
{
	...
	else
	{
        ...
        //这是一个指针数组
        /*delete运算符只是释放了this->m_EmpArray[ret]指向的内存空间，但并没有改变指针本身的值。
        所以，你仍然可以使用这个指针去赋值，只不过这样做可能会导致内存泄漏或者访问非法的内存地址。
        为了避免这种情况，你可以在delete之后，将指针置为nullptr，表示它不再指向任何有效的内存空间。
        */
		if (ret != -1)
		{ 
			//查找到编号的职工
